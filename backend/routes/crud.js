...
router.post('/:resource', async (req, res) => {
  ...
  // Debug: show allowed columns and incoming payload keys to help diagnose "No valid data provided"
  try { console.debug('[crud] allowedCols for table', table, ':', allowedCols); console.debug('[crud] incoming payload keys:', Object.keys(data || {})); } catch (e) {}
    const colInfo = Object.fromEntries(colRows.map(r => [r.Field, r]));
    // Attempt to map incoming keys to allowed table columns using case-insensitive and camelCase->snake_case heuristics
    const mappedData = {};
    const allowedLower = Object.fromEntries(allowedCols.map(c => [c.toLowerCase(), c]));
    for (const k of Object.keys(data || {})) {
      const v = data[k];
      const kLower = String(k).toLowerCase();
      // direct match
      if (allowedLower[kLower]) {
        mappedData[allowedLower[kLower]] = v;
        continue;
      }
      // try snake_case from camelCase: e.g., montoMes -> monto_mes
      const snake = String(k).replace(/([A-Z])/g, '_$1').toLowerCase();
      if (allowedLower[snake]) {
        mappedData[allowedLower[snake]] = v;
        continue;
      }
      // try removing non-alphanum and underscores
      const plain = kLower.replace(/[^a-z0-9]/g, '');
      const matched = Object.keys(allowedLower).find(a => a.replace(/[^a-z0-9]/g, '') === plain);
      if (matched) {
        mappedData[allowedLower[matched]] = v;
        continue;
      }
      // last resort: keep original key (will be filtered out later)
      mappedData[k] = v;
    }

    const keys = Object.keys(mappedData).filter(k => allowedCols.includes(k));
  ...
  const required = colRows.filter(r => r.Null === 'NO' && !r.Extra.includes('auto_increment') && r.Default == null).map(r => r.Field);
    ...
    for (const reqCol of required) {
      ...
      const presentInPayload = keys.includes(reqCol) && mappedData[reqCol] !== undefined && mappedData[reqCol] !== null;
      let has = false;
      if (presentInPayload) {
        ...
        if (t.includes('char') || t.includes('text')) {
          ...
          has = String(mappedData[reqCol]).trim() !== '';
        }
      }
...
router.put('/:resource/:id', async (req, res) => {
  ...
  const [colRows] = await pool.query(`DESCRIBE \`${table}\``);
    ...
    const colInfo = Object.fromEntries(colRows.map(r => [r.Field, r]));
    // Map incoming keys for updates too (case-insensitive / camelCase -> snake_case)
    const mappedDataUpd = {};
    const allowedLowerUpd = Object.fromEntries(allowedCols.map(c => [c.toLowerCase(), c]));
    for (const k of Object.keys(data || {})) {
      const v = data[k];
      const kLower = String(k).toLowerCase();
      if (allowedLowerUpd[kLower]) { mappedDataUpd[allowedLowerUpd[kLower]] = v; continue; }
      const snake = String(k).replace(/([A-Z])/g, '_$1').toLowerCase();
      if (allowedLowerUpd[snake]) { mappedDataUpd[allowedLowerUpd[snake]] = v; continue; }
      const plain = kLower.replace(/[^a-z0-9]/g, '');
      const matched = Object.keys(allowedLowerUpd).find(a => a.replace(/[^a-z0-9]/g, '') === plain);
      if (matched) { mappedDataUpd[allowedLowerUpd[matched]] = v; continue; }
      mappedDataUpd[k] = v;
    }
    const keys = Object.keys(mappedDataUpd).filter(k => allowedCols.includes(k));
    if (keys.length === 0) return res.status(400).json({ message: 'No valid data provided' });
